...
void resizewin(const Arg *arg) {
    ...
    if (!XQueryPointer(dpy, root, &dummy, &dummy, &di, &di, &di, &di, &dui))
        return;

    horizcorner = (ev.xbutton.x_root > ocx + c->w / 2);
    vertcorner = (ev.xbutton.y_root > ocy + c->h / 2);
    ...
    do {
        ...
        case MotionNotify:
            ...
            nw = MAX(ev.xmotion.x_root - (horizcorner ? ocx : c->x), 1);
            nh = MAX(ev.xmotion.y_root - (vertcorner ? ocy : c->y), 1);
            if (horizcorner)
                nw = MAX(ev.xmotion.x_root - ocx, 1);
            if (vertcorner)
                nh = MAX(ev.xmotion.y_root - ocy, 1);
